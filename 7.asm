...
parametersEnded:
	inc si
	mov byte ptr [si], 0        ;заносим 0 в конец(ASCIZ-строка имеет такую структуру)
...
getNumberLength:                         ;проверка длины слова
        xor cx, cx
        mov di, offset length
        mov cl, [di] 
        cmp cl, 0
        je invalidInput      	
convert:								 ;конвертация из символов в цифры с проверкой
        xor ax, ax
    	lodsb
        cmp al, '-'	
    	je invalidInput	
